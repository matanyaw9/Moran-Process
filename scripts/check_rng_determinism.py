"""
Verify RNG determinism across four test batches:
  - Batches 1 & 2: batch_seed=42  -> manifests and results must be identical
  - Batches 3 & 4: batch_seed=None -> manifests and results must differ (from each other and from 1/2)

Run with:
    uv run python scripts/check_rng_determinism.py


Sanity check run 2026-06-03 against batches randomness-1 through randomness-4.
Verifies that batch_seed=42 produces identical results across two runs,
and that batch_seed=None produces independent results.
...
BATCHES = {
    1: "2026-06-03_randomness-1",
    2: "2026-06-03_randomness-2",
    3: "2026-06-03_randomness-3",
    4: "2026-06-03_randomness-4",
}
...
# ---------------------------------------------------------------------------
# 5. Zoo graph hashes: note on determinism
# ---------------------------------------------------------------------------
...
all_same = all(hashes[i] == hashes[1] for i in range(2, 5))
if all_same:
    print(f"  NOTE: All 4 zoos contain identical graphs ({len(hashes[1])} graphs, same wl_hashes).")
    print("  The random graphs in this batch use explicit integer seeds (s0-s9 in their names),")
    print("  so they are topology-deterministic and produce the same wl_hash across all batches.")
    print("  batch_seed only controls the simulation RNG, not graph generation.")
else:
    for i in range(2, 5):
        only_in_1 = hashes[1] - hashes[i]
        only_in_i = hashes[i] - hashes[1]
        print(f"  Batch 1 vs {i}: only_in_1={len(only_in_1)}, only_in_{i}={len(only_in_i)}")
