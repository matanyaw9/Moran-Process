...
def next_generation(
    graph_zoo: list[PopulationGraph], size: int, rng: np.random.Generator = None
):
    new_generation = []
...
def test_random_seed():
    seed = 42
    G1 = PopulationGraph.random_connected_graph(
        30, 29, name=f"random_1_seed_{seed}", seed=seed, labeled_edges=False
    )
    G2 = PopulationGraph.random_connected_graph(
        30, 29, name=f"random_2_seed_{seed}", seed=seed, labeled_edges=False
    )
    ...
    assert (
        G1.wl_hash == G2.wl_hash
    ), "WL hash should be the same if they are created with the same seed!"

    # now let's mutate
    G1_mutate = G1.mutate_graph(seed=seed)
    G2_mutate = G2.mutate_graph(seed=seed)
    G1_mutate.draw(
        filename=f"./tmp_images/random_check/{G1_mutate.name}.png", with_labels=True
    )
    G2_mutate.draw(
        filename=f"./tmp_images/random_check/{G2_mutate.name}.png", with_labels=True
    )
    assert (
        G1_mutate.wl_hash == G2_mutate.wl_hash
    ), "WL hash should be the same if they are mutated from the same seed graph!"
