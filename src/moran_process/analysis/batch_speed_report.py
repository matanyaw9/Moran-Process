...
def batch_speed_report(batch_name: str, df: pd.DataFrame, batch_root: Path) -> None:
    ...
    print(
        f"  Avg job run time : {_fmt_sec(avg_run_sec)}  |  Max: {_fmt_sec(max_run_sec)}"
    )
    print(
        f"  Wall throughput  : {avg_speed_k:.1f} k steps/s avg  |  min {min_speed_k:.1f}  |  max {max_speed_k:.1f}   (steps / wall-clock, incl. startup/IO)"
    )
    ...
    if has_dur:
        ...
        sim_thru_M = (
            (total_steps / total_sim_sec / 1e6) if total_sim_sec else float("nan")
        )
        wall_thru_M = (
            (total_steps / total_run_sec / 1e6) if total_run_sec else float("nan")
        )
        speedup = (sim_thru_M / wall_thru_M) if wall_thru_M else float("nan")
        print(f"  -- engine throughput (from `duration`, sim-only) --")
        print(
            f"  Pure sim time    : {_fmt_sec(total_sim_sec)}  ({sim_pct:.2f}% of wall; rest is startup/IO overhead)"
        )
        print(
            f"  Sim throughput   : {sim_thru_M:.2f} M steps/s   (engine-only, what the C++ core accelerates)"
        )
        print(
            f"  Overhead-masking : sim is {speedup:.0f}x faster than wall throughput; per-job startup hides it"
        )

    if has_mem:
        print(
            f"  Peak RAM per job : avg {max_mem_col.mean():.0f} MB  |  min {max_mem_col.min():.0f} MB  |  max {max_mem_col.max():.0f} MB"
        )
        print(
            f"  Avg  RAM per job : avg {avg_mem_col.mean():.0f} MB  |  min {avg_mem_col.min():.0f} MB  |  max {avg_mem_col.max():.0f} MB"
        )
    ...
    axes[1].hist(
        job_stats["steps_per_sec"] / 1e3, bins=30, color="seagreen", edgecolor="white"
    )
