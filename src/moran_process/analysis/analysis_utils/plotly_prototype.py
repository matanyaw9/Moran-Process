        ...
        go.Scattergl(
            ...
            marker=dict(
                ...
                colorbar=dict(
                    ...
                    len=0.55,
                    y=1.0,
                    yanchor="top",  # top ~55%, mirrors the static stack
                ),
            ),
...
def _hover(plot_df, x_prop, y_outcome):
    ...
    template = id_lines + f"{x_prop}: %{{x}}<br>{y_outcome}: %{{y:.4g}}<extra></extra>"
...
def plot_outcome_vs_property_plotly(
    ...
    ylabel = (
        "Fixation Probability (P_fix)"
        if is_prob
        else y_outcome.replace("_", " ").title()
    )
        ...
        for i, r in enumerate(r_values):
            fig.add_trace(
                go.Scatter(
                    ...
                    name=f"r = {r:g}",
                    legendrank=1000 + i,
                    legendgroup="r_legend",
                    legendgrouptitle_text="r (mutant relative fitness)",
                    marker=dict(
                        size=8,
                        symbol=r_symbol[r],
                        color="dimgray",
    ...
    fig.update_layout(
        ...
        template="plotly_white",
        width=round(DEFAULT_FIG_SIZE[0] * 100),
        height=round(DEFAULT_FIG_SIZE[1] * 100),
        margin=dict(r=180),
...
def _demo_dataframe(seed=0):
    ...
    category = rng.choice(["Random", "Mammalian", "Avian"], size=n, p=[0.8, 0.1, 0.1])
    avg_degree = rng.choice([2, 3, 4, 5, 6], size=n)  # few unique -> triggers violins
    return pd.DataFrame(
        dict(
            n_nodes=n_nodes,
            prob_fixation=prob_fixation,
