...
def plot_property_effect(df, x_prop, y_outcome='prob_fixation', color_dict=COLOR_DICT, unique_threshold=10):
    """
    Plots a specific graph property against an evolutionary outcome.
    - If x_prop has many values: Standard Scatter Plot.
    - If x_prop has few values: Violin Plot + Jittered Scatter Overlay (preserving shape/style).
    """
    ...
    # --- 2. Data Preparation ---
    # We create a working copy to avoid SettingWithCopy warnings on the original df
    plot_df = df.copy()
    
    # Round X-values if numeric for cleaner grouping
    if pd.api.types.is_numeric_dtype(plot_df[x_prop]):
        plot_df['x_rounded'] = plot_df[x_prop].round(3)
    else:
        plot_df['x_rounded'] = plot_df[x_prop]
        
    unique_vals = sorted(plot_df['x_rounded'].unique())
    ...
    if unique_count <= unique_threshold:
        ...
        # A. Create an explicit mapping to integers 0, 1, 2... 
        # This ensures Violin and Scatter use the exact same coordinate system.
        x_map = {val: i for i, val in enumerate(unique_vals)}
        plot_df['x_pos'] = plot_df['x_rounded'].map(x_map)
        
        # B. Draw Violin (Bottom Layer)
        # We plot using the integer positions 'x_pos'
        sns.violinplot(
            data=plot_df,
            x='x_pos',
            y=y_outcome,
            inner=None,          # No inner bars, we will add scatter
            color='whitesmoke',  # Neutral background
            linewidth=1,
            saturation=0.5,
            width=0.7            # Width of the violin blob
        )

        # C. Draw Scatter Overlay with Jitter (Top Layer)
        # Add random noise to the integer positions
        jitter_strength = 0.1
        noise = np.random.uniform(-jitter_strength, jitter_strength, size=len(plot_df))
        plot_df['x_jittered'] = plot_df['x_pos'] + noise
        ...
        sns.scatterplot(
            ...
            style='r',          # Shape by r (Works because this is a real scatterplot)
            palette=color_dict,
            s=50,               # Size
            alpha=0.85,
            edgecolor='w',
            linewidth=0.5,
            zorder=2            # Force on top
        )

        # D. Fix X-Axis Labels
        # We plotted on 0, 1, 2... so we must rename the ticks back to the real values
        plt.xticks(
            ticks=range(len(unique_vals)), 
            labels=unique_vals
        )
        plt.xlabel(x_prop.replace('_', ' ').title())

    else:
        # === SCATTER MODE (Standard) ===
        sns.scatterplot(
            data=df,
            x=x_prop,
            y=y_outcome,
    ...
    # Legend: Place outside top right
